...
✓ File modifications
✓ Test runs, Flip-only device launches (see section 9), adb logcat/reverse/install -- **coordinate taps only per section 9 (Flip foreground)**
```
...
#### Coordinate taps (`adb shell input tap`) -- Flip foreground only

Blind coordinate taps are **ALLOWED** when Flip is the resumed foreground app. Agents must know they are tapping **inside Flip's UI**, not the launcher, system chrome, or other apps.

**REQUIRED before any tap:**

```bash
adb shell dumpsys window | findstr /i mCurrentFocus
```

(or equivalent) -- proceed only if the line contains `social.flip.app`. If focus is wrong:

```bash
adb shell am start -n social.flip.app/.MainActivity
```

Wait for Flip to settle, then re-check focus before tapping.

**Valid tap targets (Flip UI only):** bottom tab bar, Flip It shutter, inbox rows, in-app buttons -- never launcher, Settings, Chrome, or notification shade.

**Reference tab bar coords (device R3GL10HN64A, Samsung S26 Ultra class, y~2870):** Home `288 2870`, Explore `576 2870`, Flip It `864 2870`, Inbox `1152 2870`, Profile `1440 2870`. Resolution, nav bar, and keyboard change hit targets; re-verify focus every session; do not tap when unsure.

#### Forbidden

- **NEVER** `adb shell input tap` when `mCurrentFocus` does **not** contain `social.flip.app` (blind taps on home screen or other apps open wrong UIs).
- **NEVER** coordinate taps intended to open or interact with non-Flip packages (launcher, Settings, Chrome, Samsung apps, system UI).
...
Same check as coordinate taps (required every time before `input tap`):

```bash
adb shell dumpsys window | findstr /i mCurrentFocus
```

Proceed only if the line contains `social.flip.app`. If not, run `am start -n social.flip.app/.MainActivity`, wait, and re-check.
...
**Last Updated:** 2026-06-24 (section 9: Flip-foreground coordinate taps allowed; issue #17)
...
3. Tap **Explore**, **Flip It**, **Inbox**, **Profile** (user taps preferred; agents: confirm `social.flip.app` foreground then Flip-only `input tap` per section 9) -- expect crash unless latest layout fix verified.
...
3. Tap **Home**, **Explore**, **Flip It**, **Inbox**, **Profile** (agents: `am start -n social.flip.app/.MainActivity`, verify `mCurrentFocus` → `social.flip.app`, then Flip-only `input tap` **or** user taps) -- each must render without red screen
...
| 2026-06-24 ~04:19 UTC | **Cursor (subagent)** | Stop restoring native queueMicrotask in withQueueMicrotaskShadow finally (root cause: RN stub restored after routingQueue.run); installRoutingQueuePatch on rootApp mount. Device verify used **coordinate adb taps** (Flip foreground only -- now formalized in section 9). | **Y** (R3GL10HN64A) -- repeat only with focus check + Flip UI targets |
