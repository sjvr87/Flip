...
export default function LinkifiedCaption({
    ...
    const onMeasureNaturalText = useCallback((event: { nativeEvent: { lines?: unknown[] } }) => {
        setNaturalLineCount(event.nativeEvent.lines?.length ?? 0);
    }, []);
    ...
    const needsMore = canDecide && naturalLineCount > 1;
    ...
    return <Text style={style}>{renderCaption()}</Text>;
}
...
const styles = StyleSheet.create({
    ...
    measureItem: { flexDirection: 'row', alignItems: 'center' },
    inlineRow: {
        width: '100%',
        flexDirection: 'row',
        alignItems: 'baseline',
    ...
    moreText: { fontSize: 14, fontWeight: '700', color: '#fff', opacity: 0.7 },
    moreIcon: { marginLeft: 2, opacity: 0.7, paddingTop: 2, color: '#fff' },
    moreIconMeasure: { marginLeft: 2, opacity: 0.7, color: '#fff' },
});
