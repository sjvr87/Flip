...
function postToComment(post: AppBskyFeedDefs.PostView, parentId?: string): FlipComment {
  ...
  const record = post.record as { text?: string; facets?: import('@atproto/api').RichText['facets'] }
...
function paginate<T>(items: T[], pageParam: string | null | false): { data: T[]; next_cursor: string | null } {
...
async function toggleCommentLike(commentUri: string, like: boolean): Promise<{ liked: boolean; likes: number }> {
  const agent = getAgent()
