...
function getImageEmbedFromRecord(post: AppBskyFeedDefs.PostView): ImageEmbedView | null {
...
function getImageEmbedFromView(post: AppBskyFeedDefs.PostView): ImageEmbedView | null {
    ...
    if (embed.$type === 'app.bsky.embed.recordWithMedia#view' && 'media' in embed && embed.media) {
...
export function postToFlipTextPost(feedItem: AppBskyFeedDefs.FeedViewPost): FlipTextPost | null {
...
export function postsToFeedPage(
    ...
    data: FlipVideo[];
    meta: FlipVideo extends never ? never : import('./types').FlipFeedPage['meta'];
} {
    const data = items
