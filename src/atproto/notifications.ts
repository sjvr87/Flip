...
function matchesActivityFilter(notification: FlipNotification, filter: string): boolean {
    switch (filter) {
        case 'followers':
...
export async function fetchNotifications({
    ...
    data: FlipNotification[];
    meta: { unread_counts: UnreadCounts; next_cursor?: string | null };
}> {
...
async function markNotificationsSeen(seenAtInput: unknown): Promise<void> {
    ...
    await withAuthenticatedFetch(() => getAgent().app.bsky.notification.updateSeen({ seenAt }));
}
...
export async function notificationTypeMarkAllAsRead(
    _type: string,
