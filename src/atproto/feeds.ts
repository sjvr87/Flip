...
function logFeedFetch(
    ...
    const nonFollow = filter?.active ? countNonFollowVideos(videos, filter.dids) : videos.length;
    console.log(
...
async function mergeForYouSuggestions(
    ...
    try {
        ...
        for (const actor of res.data.actors) {
            if (
                followingFilter.dids.has(actor.did) ||
                (actor.handle && followingFilter.dids.has(actor.handle))
            ) {
                continue;
            }
...
const BLUESKY_WHATS_HOT = 'at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.feed.generator/whats-hot';
...
function getForYouFeedUri(): string | undefined {
    return normalizeFeedUri(
        Constants.expoConfig?.extra?.flipForYouFeed || process.env.EXPO_PUBLIC_FLIP_FORYOU_FEED,
    );
}
...
async function fetchForYouBySource(
    ...
    switch (sourceId) {
        case 'custom':
            return fetchGeneratorFeed(
                customUri || BLUESKY_THE_VIDS,
                cursor ?? false,
                'for you feed',
            );
        case 'search-latest':
...
export async function fetchForYouFeed({
    ...
    try {
        ...
        try {
            ...
            console.warn(
                '[feed] for you video search failed, falling back to discover:',
                searchError,
            );
            const followingFilter = await ensureDiscoveryFollowingFilter();
            const { videos, nextCursor } = await fetchDiscoveryUntilNonFollow(
...
function mediaPageWithResolvedPost(
    ...
    const items = !hasTarget ? [{ post: resolvedPost, reply: undefined }, ...feedItems] : feedItems;
...
export async function fetchPostForViewer(rawUri: string): Promise<FlipVideo | FlipTextPost | null> {
